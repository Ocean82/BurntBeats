...
// Add example track API route
// Example code to generate a melody and serve it as an example track
app.get("/api/example-track", async (req: Request, res: Response) => {
  try {
    // Dynamically import melody generator
    const { MelodyGenerator } = await import('./melody-generator');
    const melodyGenerator = new MelodyGenerator();

    // Generate a melody with full configuration
    ...
    // Access the results
    console.log(`Generated ${melody.noteCount} notes`);
    console.log(`Audio path: ${melody.audioPath}`);
    console.log(`Key: ${melody.audioFeatures.key}`);
    console.log(`Phrases: ${melody.phrases.length}`);

    // Send the audio path back to the client so it can be played
    res.json({
      success: true,
      audioPath: melody.audioPath,
      message: "Example track generated successfully"
    });
  } catch (error: any) {
